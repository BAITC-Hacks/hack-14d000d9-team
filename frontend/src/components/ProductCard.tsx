...
export function ProductCard({ product, compact, onAsk, onBuy }: Props) {
  ...
  return <article className={`product-card ${compact ? 'product-card--compact' : ''}`}>
    ...
    <div className="product-info"><span className="article">Арт. {product.article}</span>
      <a className="product-name" href={product.url} target="_blank" rel="noreferrer">{product.name.replace(/^\*+/, '')}<ExternalLink size={12} /></a>
      ...
      <strong className="price">{money(product.price)}</strong><small>Цена из выгрузки</small>
      {product.description && <div className="product-details">
        ...
        <details><summary>Остатки по складам · {product.quantity} шт.</summary><dl>{product.stores?.map((store) => <div key={store.id}><dt>{store.name}</dt><dd>{store.quantity} шт.</dd></div>)}</dl><small>Снимок данных. Перед покупкой требуется проверка.</small></details>
      </div>}
