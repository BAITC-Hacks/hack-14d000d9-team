import { loadCatalog } from '../services/backendApi';
...
export function ChatPage() {
  ...
  const [catalogStatus,setCatalogStatus] = useState(apiEnabled?'Загружаем каталог…':'Локальная демо-выборка');
  useEffect(()=>{if(apiEnabled)loadCatalog().then(s=>setCatalogStatus(`${s.real_products} товаров API + ${s.demo_products} синтетических · ${s.catalog_mode==='live'?'карточки онлайн':'снимок API'}`)).catch(()=>setCatalogStatus('Сервер недоступен. Показан локальный снимок.'));},[]);
  ...
  return <div className={embedded ? 'embedded' : 'storefront'}>
    {!embedded && <>
      ...
      <header className="store-header"><a className="brand" href="/"><Zap size={32} fill="currentColor" /><span>EKT<small>ekt.kz</small></span></a><label className="catalog-search"><Search size={20} /><input aria-label="Поиск по каталогу" placeholder="Название или артикул товара" value={query} onChange={(event) => setQuery(event.target.value)} /></label><a className="cart-nav" href="/cart">Демо-корзина →</a></header>
      <main className="catalog-main"><div className="catalog-heading"><div><span className="eyebrow">EKT.KZ</span><h1>Электротехническая продукция</h1></div></div>
        <div className="results-toolbar"><span>Найдено: {found.length}</span><span>{catalogStatus}</span></div>
